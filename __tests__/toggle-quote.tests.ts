...
describe("Examining toggle-blockquote-at-current-indentation", () => {
  ...
  test("Adds and removes blockquote from single line that begins with whitespace with cursor at beginning of line", async () => {
    ...
    await toggleQuote(view, defaultPrefix);
    expect(JSON.stringify(view)).toEqual(
      '{"editor":{"content":["    > Lorem ipsum dolor sit amet, consectetur adipiscing elit.","Sed venenatis lectus et leo viverra, ac viverra purus rutrum.","","Etiam semper massa ut est faucibus, eu luctus arcu porttitor."],"selection":[{"line":0,"ch":6},{"line":0,"ch":6}]}}'
    );

    await toggleQuote(view, defaultPrefix);
    expect(JSON.stringify(view)).toEqual(
      '{"editor":{"content":["    Lorem ipsum dolor sit amet, consectetur adipiscing elit.","Sed venenatis lectus et leo viverra, ac viverra purus rutrum.","","Etiam semper massa ut est faucibus, eu luctus arcu porttitor."],"selection":[{"line":0,"ch":4},{"line":0,"ch":4}]}}'
    );
  });
});
